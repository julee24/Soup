...
<html lang="en">
    <head>
        ...
        <link rel="stylesheet" href="css/studylog.css?ver=2"/>
    ...
    <main class="main">
      <div class="responsive-wrapper">
        <div class="main-header">
            <h2>📒<?php echo $g_name;?>의 Study Diary</h2>
        </div>
        ...
        <!-- <div class="profile">
            <div class="column">
                <div class="circle">
                    <div class="selfie">
                      <img src="css/img/profile2.png" class="picture">
                    </div>
                </div>
            </div>
            <div class="column">
                <h1 name="nickname">Yeja Lim</h1>
                <p class="chat">나 오늘은 공부한다...</p>
            </div>
            <div class="column">
                <a class="startBlog">작성하기</a>
            </div>
        </div> -->
        <div class="blogs">
            <ul>
            ...
            $id=$_SESSION['id'];
            $connect = mysqli_connect("localhost","root","yewwey1105","newdb") or die ("Can't access DB");
            $sql= "SELECT * FROM diary where id='$id' order by writetime desc;";
            $confirm = mysqli_query($connect, $sql);             
            // $array = mysqli_fetch_array($confirm);
            ...
            {   $content = $row['text'];
                ...
                // $date = array_slice($dateArray,0,9);
                $date = explode(" ", $dateArray);
                $timeArray = $row["savetime"];
                $time = explode(":",$timeArray);
                $hours=$time[0];
                $minutes=$time[1];
                $seconds=$time[2];
                $hour1=substr($hours,0,1);
                $hour2=substr($hours,1,1);
                $minute1=substr($minutes,0,1);
                $minute2=substr($minutes,1,1);
                $second1=substr($seconds,0,1);
                $second2=substr($seconds,1,1);
               
                $image = $row['photo'];
                ?>
                <li class="booking-card" style="background-image: url(<?php if($image) {echo 'data:image/png;base64,'.base64_encode($image).'';}
                else {
                    //폴더에있는 사진들 넣기
                ...
                ?>)" >
                  <div class="book-container">
                    <div class="content">
                        ...
                        <button type="submit" class="date" data-modal-trigger="trigger-1"><i class="fa fa-fire" aria-hidden="true"></i><?php echo $date[0];?></button>
                        </form>
                    </div>
                  </div>
                  <div class="informations-container">
                    <!-- <h2 class="title">오늘도 열심히 공부하는 날!</h2> -->
                    <!-- <p class="sub-title">✅ 토익 1장 끝내기</p> -->
                    <p class="price">⏰ <?php if($hour1 == "0"){
                        if($hour2!="0"){
                            echo $hour2,"H";
                        }}else {
                            echo $hours,"H";
    ...
    <!-- <script>
        ...
        function modalEvent(button) {
            button.addEventListener('click', () => {
                ...
                const write = button.getAttribute('write');
