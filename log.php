<?php
    ini_set('display_errors', '0');
    $is_logged = 'NO';
    session_start();
    $is_logged = $_SESSION['is_logged'];
    $_SESSION['confirm'] = 'NO';
    if(!isset($_SESSION)) {
        session_start();
        $is_logged = $_SESSION['is_logged'];
    }
?>
<!DOCTYPE html>
<html lang="en">
    <head>
        <meta charset="UTF=8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <link rel="stylesheet" href="css/studylog.css?ver=3"/>
        <!-- <link rel="stylesheet" href="css/index.css"/> -->
        <title>스프:공스타그램</title>
    </head>
    <body>
      <header class="header">
        <div class="header-content responsive-wrapper">
            <div class="header-logo">
                <a href="index.php">
                    <!-- <div>
                        <img src="https://assets.codepen.io/285131/untitled-ui-icon.svg" />
                    </div> -->
                    <!-- <h3>Soup:스터디프렌즈</h3> -->
                    <img src="img/logo1.png" />
                </a>
            </div>
            <div class="header-navigation">
                <nav class="header-navigation-links">
                    <ul>
                        <li><a href="study.php">공부하기</a></li>
                        <li>
                            <a href="#">나의 공부</a>
                            <ul class="submenu">
                                <li><a href="studylog.php">공부 일기</a></li>
                                <li><a href="planner.php">공부 계획</a></li>
                            </ul>
                        </li>
                        <li><a href="mypage.php">마이페이지</a></li>
                        <li>
                            <a href="#">고객센터</a>
                            <ul class="submenu">
                                <li><a href="question.php">자주하는 질문(FAQ)</a></li>
                                <li><a href="myqna.php">1:1 문의</a></li>
                            </ul>
                        </li>
                    </ul>
                </nav>
                <div class="header-navigation-actions">
                    <?php
                        if($is_logged=='YES') {
                        ?>
                        <a href="logout.php" class="button">
                            <i class="ph-lightning-bold"></i>
                            <span>Logout</span>
                        </a>
                        <a href="#" class="avatar">
                        <?php
                        $id = $_SESSION['id'];
                        $connect = mysqli_connect("localhost","root","yewwey1105","newdb") or die ("Can't access DB");
                        $sql= "SELECT * FROM users where id='$id'";
                        $confirm = mysqli_query($connect, $sql);             
                        $array = mysqli_fetch_array($confirm);
                        $profile = $array['profile'];
                        if ($profile == ''){
                            echo '<img src="data:image/png;base64,'.$profile.'">';
                        } else {
                            echo '<img src="data:image/png;base64,'.base64_encode($profile).'">';
                        }
                        $_SESSION['profile'] = $profile;
                        ?>
                        </a>
                        <?php
                        } else {
                        ?>
                        <a href="login.php" class="button">
                            <i class="ph-lightning-bold"></i>
                            <span>Login & Sign up</span>
                        </a>
                        <?php
                        }
                        ?>
                    </div>
            </div>
            <a href="#" id="menu-button" class="button" >
                <i class="ph-list-bold"></i>
                <span>Menu</span>
            </a>
            <div class="menu-wrap">
                <nav class="menu-nav">
                <ul>
                    <li><a href="study.php">공부하기</a></li>
                    <li>
                        <a href="#">나의 공부</a>
                        <ul class="submenu">
                            <li><a href="studylog.php">공부 일기</a></li>
                            <li><a href="planner.php">공부 계획</a></li>
                        </ul>
                    </li>
                    <li><a href="mypage.php">마이페이지</a></li>
                    <li>
                        <a href="#">고객센터</a>
                        <ul class="submenu">
                            <li><a href="question.php">자주하는 질문(FAQ)</a></li>
                            <li><a href="myqna.php">1:1 문의</a></li>
                        </ul>
                    </li>
                </ul>
                </nav>
            </div>
            
        </div>
    </header>
    <script>
            function myFunction() {
            var check = confirm("삭제하시면 복구되지않습니다.\n정말로 삭제하시겠습니까?");
            if(check){
                return true;
            }else{
                return false;
            }
            }
    </script>
    <main class="main">
      <div class="responsive-wrapper">
        <?php
            $modalWritetime=$_GET['write'];
            $diaryid = $_GET['user'];
            $connect = mysqli_connect("localhost","root","yewwey1105","newdb") or die ("Can't access DB");
            $sql= "SELECT * FROM diary where id='$diaryid'and writetime='$modalWritetime'";
            $confirm = mysqli_query($connect, $sql);
            $array =  mysqli_fetch_array($confirm);
            $modalContent = $array['text'];
            $modalPhoto = $array['photo'];
            $deletetime = $modalWritetime;
            $modalWrite = explode(" ", $modalWritetime);
            $writeDate = explode("-",$modalWrite[0]);
            $year = $writeDate[0];
            $month = $writeDate[1];
            $day = $writeDate[2];

            $time = explode(":",$modalWrite[1]);
            $hour=$time[0];
            $minute=$time[1];
            $hour1=substr($hour,0,1);
            $hour2=substr($hour,1,1);
            $inthour =(int)$hour;
            // mysqli_close($connect);
            $query= "SELECT * FROM users where id='$diaryid'";
            $confirm = mysqli_query($connect, $query);
            $user =  mysqli_fetch_array($confirm);
            $username = $user['name'];
            $userprofile = $user['profile'];
        ?>
        <div class="main-header">
            <h2>📒<?php echo $username;?>의 Study Diary</h2>
        </div>
        <div class="sub-header">
            <span></span>
                <!-- ☎️ -->
        </div>
        
        <div class="content">
        <div class="card">
            <form action="delete.php" onsubmit="return myFunction();">
                <input type="hidden" name="deleteDiary" value="<?php echo $deletetime;?>">
                <button type="submit" class="btn"<?php if($diaryid != $id) {?> style="display:none;" <?php } ?>></button>
            </form>
            
            <div class="card-header">
                <img src="<?php
                    if ($modalPhoto){
                        echo 'data:image/png;base64,'.base64_encode($modalPhoto).'';
                    } else {
                        $list = [ "1.png","2.jpg","3.jpg","4.jpg"];
                
                        $length = count($list);
                        $sel = rand(0, $length -1);
                        echo 'studyimage/'.$list[$sel];
                    } 
                ?>">
            
            <div class="user">
                <?php
                    if ($userprofile == ''){
                        echo '<img src="data:image/png;base64,'.$userprofile.'">';
                    } else {
                        echo '<img src="data:image/png;base64,'.base64_encode($userprofile).'">';
                    } 
                ?>
                <div class="user-info">
                <h4><?php echo $username;?></h4>
                <small><?php echo $year,"/",$month,"/",$day;
                if ($hour == "00") {
                    echo " AM 12:";
                } else if($hour1 == "0") {
                    echo " AM ",$hour,":";
                } else {
                    if($hour=="12") {
                        echo " PM 12:";
                    }
                    else if($inthour > 12) {
                        $hour3 = $inthour%12;
                        echo " PM ",$hour3,":";
                    } else {
                        echo " AM ",$hour,";";
                    }
                }
                echo $minute;?></small>
                </div>
            </div>      
            </div>
            <div class="card-body">
            <!-- <span class="tag tag-teal">Technology</span> -->
                <h4>
                    <?php echo $modalContent;?>
                </h4>
            </div>
        </div>
        </div>

      </div>

    </main>
    <footer class="footer">
      <ul class="f-list">
          <li>회사소개</li>
          <li>이용약관</li>
          <li>개인정보처리방침</li>
          <li>운영정책</li>
          <li>주요서비스</li>
      </ul>
      <div class="name">©StudyFriends:스프</div>
    </footer>
        <!-- <script src="js/studylog.js"></script> -->
        
    </body>